...
import dns from 'dns'
import { app, shell, BrowserWindow, Notification, dialog, ipcMain, nativeImage } from 'electron'
...
function initMain() {
    ...
    mainProcess.setFunctions({
        ...
        openXMLEditor: (bridge?: boolean) => {openXMLEditor(bridge)},
        openList: () => {openList()},
        ...
        openSaveDialog: (defaultName: string) => openSaveDialog(defaultName),
        openConsole: () => {openConsole()},

        saveBackup: (reloadAfter?: boolean) => {saveBackup(reloadAfter)},
        copyBackup: () => {copyBackup()},
        resetConfig: (noReload?: boolean) => {resetConfig(noReload)},
        restoreInitial: () => {restoreInitial()},
        saveConfig: () => {saveConfig()},
        saveInitialSum: () => {saveInitialSum()},
        checkUpdate: (whateverCheck?: boolean) => {checkUpdate(whateverCheck)},
        update: () => {update()},
        unpackFiles: (popup?: boolean) => {unpackFiles(popup)},
        enableDevTools: () => {settings.devTools = true},
        disableDevTools: () => {settings.devTools = false}
    })
}

function saveModSum(modId: string, path: string) {
...
function download(params: IDownloadParams, cb: (data?: any)=>any) {
    if (params.array) {
        if (params.isRoot) {
...
function checkPathToDelete(path: string, map: IUpdateMap) {
    const toRemove = []
...
function checkMap(map: IUpdateMap) {
...
function checkUpdate(whateverCheck?: boolean) {
...
function permToFile(path: string) {
    try {
...
function showNotification(titleKey: TKeys, messageKey: TKeys) {
    return new Promise(resolve => {
...
function openXMLEditor(bridge?: boolean) {
...
function openDownload(popup?: boolean) {
    const beforeWindow = windows.currentWindow
    const wind = <IDownloadWindow>createWindow({
...
function openUpdateMessage(version: string) {
    const beforeWindow = windows.currentWindow
    const wind = createWindow({
...
function seeExported() {
    ...
    if (path) {path2 = path[0]}
    else return

    const object = JSON.parse(readFileSync(path2).toString())
    const result = []
...
function unpackFiles(popup?: boolean) {
    return new Promise(resolve => {
...
function unpackMod(absolutePath: string) {
    return new Promise(resolve => {
        const modName = join(paths.modsTemp, basename(absolutePath, '.pak'))
        if (!permToFile(absolutePath)) resolve(null)
        ...
        mkdirSync(modName);
        archiver.unpack(absolutePath, modName, true).then(() => resolve(null))
    })
}

function saveBackup(reloadAfter?: boolean) {
    return new Promise(resolve => {
        unpackFiles().then(() => {
...
function createWindow(args: ICreateWindowAttributes) {    
...
function resetConfig(noReload?: boolean) {
    config.paths = {
        initial: null,
        dlc: null,
        classes: null,
...
function getText(key: TKeys, returnKey: boolean=true) {
...
function getMainMenu(): IMenuTemplate[] {
    function retif(expression: any, valueToReturn: IMenuTemplate[]) {
