...
class Backup {
  ...
  async save(reload?: boolean, hideLoading?: boolean): Promise<void> {
    ...
    if (existsSync(paths.backupInitial))
      try { rmSync(paths.backupInitial) }
      catch { throw new Error($.DELETE_OLD_INITIAL_BACKUP_ERROR) }
  ...
  async recoverFromIt(): Promise<void> {
    ...
    if (existsSync(config.initial))
      try { rmSync(config.initial) }
      catch { dialogs.error($.DELETE_OLD_INITIAL_BACKUP_ERROR) }
