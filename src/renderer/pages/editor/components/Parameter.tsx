...
export default memo((props: IProps) => {
    ...
    const getDefaultValue = useCallback(() => {
        if (!defaults[item.selector] || defaults[item.selector][item.name] === undefined)
            return undefined;

        return String(defaults[item.selector][item.name]);
    }, [defaults, item]);
...
function getFromTemplates(fileDOM: CheerioAPI, item: any, templates: Cheerio<"_templates">, globalTemplates: CheerioAPI) {
...
function getValueInGlobal(templateName: string, tagName: string, globalTemplates: CheerioAPI, item: any) {
