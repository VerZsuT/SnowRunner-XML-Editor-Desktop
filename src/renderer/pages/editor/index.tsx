...
const { updateFiles, openEPFDialog, openSaveDialog } = main;
const {
    SAVING_MESSAGE,
    SUCCESS_SAVE_FILES,
    SUCCESS_RESET,
...
export default memo(() => {
    ...
    const exportFile = useCallback(() => {
        ...
        const pathToSave = openSaveDialog(basename(filePath, ".xml"));
        if (!pathToSave) {
    ...
    function addFile(mod: string, dlc: string, dom: CheerioAPI, path: string, fileType: FileType) {
        const obj = {
            dom, path, mod, dlc, fileType
        };
    ...
    function addParam(param: IIEParam) {
        const params = getParams();
    ...
    function registerReset(id: string, func: ()=>void) {
        resetList[id] = func;
    }

    function unregisterReset(id: string) {
        delete resetList[id];
    }
