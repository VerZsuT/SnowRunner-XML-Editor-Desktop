...
import { ARCHIVE_TOKEN, CONFIG_TOKEN, DIRS_TOKEN, ENV_TOKEN, FILES_TOKEN, GAME_TEXTS_TOKEN, SIZES_TOKEN } from '@utilities/di/main/tokens'
 ...
 * _main process_
 */
export class Mods extends MainArrayBase<IMod, IMod & { file: IFile }> {
	/** Архиватор. */
	@inject(ARCHIVE_TOKEN)
  ...
  async procMods() {
    ...
    for (const { file, name } of this.converted) {
      ...
      await this.archive.unpackMod(file, name)
  ...
  async findMods(dir: IDir): Promise<[IFile, name: string][]> {
    ...
    if (!await dir.exists()) {
      ...
      if (file.isExt('pak')) {
        await this.archive.unpack(file.path, tempDir.path)
  ...
  async getAllMods(): Promise<[IFile, string][]> {
    ...
    const userDir = this.dirs.new(userInfo().homedir || homedir() || env.home || '')
    const out: [IFile, string][] = []
