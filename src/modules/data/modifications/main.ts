...
import { Dir, Dirs, File, Files } from '@modules/files/main'
import Texts from '@modules/game-texts/main'
import type { IDir, IFile } from '@modules/main'
import { hasItems, MainArrayBase, processNameForFilesystem } from '@utilities/main'
...
class Mods extends MainArrayBase<IMod, IMod & { file: IFile }> {
  ...
  async getAllMods(): Promise<[IFile, string][]> {
    const userDir = new Dir(userInfo().homedir || homedir() || Env.home || '')
    const out: [IFile, string][] = []
