...
export default class XMLElement {
  ...
  after(child: string | XMLElement) {
    this.element.after(isString(child)
      ? child
      : child.element
    )
  ...
  selectAll(selector: string) {
    ...
    this.element
      .find(selector)
      .each((_, element) => void elements.push(new XMLElement(this.element.find(element).eq(0))))
